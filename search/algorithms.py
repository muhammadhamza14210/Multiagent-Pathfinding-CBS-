...
class CBSState:
    ...
    def compute_cost(self):
        ...
        for agent in range(self._k):
            cost, path = astar.search(self._starts[agent], self._goals[agent], self._constraints[agent])
            if cost == -1:
                return -1
            self._paths[agent] = path
            self._cost += cost
    ...
    def is_solution(self):
        ...
        for agent in range(self._k):
            path = self._paths[agent]
            for time_step, state in enumerate(path):
                if (state, state.get_g()) in conflict_set:
                    return False, (state, state.get_g())
                conflict_set.add((state, state.get_g()))
    ...
    def successors(self):
        ...
        for agent in range(self._k):
            ...
            for time, state in enumerate(path):
                if state == conflict_state and time == conflict_time:
                    if agent1 is None:
                        agent1 = agent
                    else:
                        agent2 = agent
        ...
        if agent1 is None or agent2 is None:
            raise Exception("Conflict information is incomplete")

        child1 = CBSState(self._map, self._starts, self._goals)
        child1._constraints = copy.deepcopy(self._constraints)
        child1.set_constraint(conflict_state, conflict_time, agent1)
        child1.compute_cost()  # Compute cost for child1

        child2 = CBSState(self._map, self._starts, self._goals)
        child2._constraints = copy.deepcopy(self._constraints)
        child2.set_constraint(conflict_state, conflict_time, agent2)
        child2.compute_cost()  # Compute cost for child2
    ...
    def __str__(self):
        return f"Cost: {self._cost}, Constraints: {self._constraints}, Paths: {self._paths}"
...
class CBS():
    def search(self, start):
        ...
        Performs CBS search for the problem defined in start.
        """
        # Create an OPEN list to store CBS states to be expanded
        open_list = []
        open_list.append(start)

        while open_list:
            current_state = min(open_list, key=lambda x: x.get_cost())
            open_list.remove(current_state)

            is_solution, _ = current_state.is_solution()
            if is_solution:
                paths = [current_state._paths[i] for i in range(current_state._k)]
                solution_cost = current_state.get_cost()
                for agent, path in enumerate(paths):
                    return paths, solution_cost
